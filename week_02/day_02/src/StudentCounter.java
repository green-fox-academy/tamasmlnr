...
public class StudentCounter {
  public static void main(String... args) {
    ...
    System.out.println("More candies than 4:");
    for (int i = 0; i < map.size(); i++) {
      if ((int) map.get(i).get("candies") > 4) {
        System.out.println(map.get(i).get("name"));
      }
    }
    System.out.println("Less candies than 5:");
    double totalAge = 0.0;
    for (int i = 0; i < map.size(); i++) {

      for (int x = 0; i < map.size(); i++) {
        if ((int) map.get(i).get("candies") < 5) {
          totalAge += Double.valueOf(map.get(i).get("age").toString());
        }
      }
